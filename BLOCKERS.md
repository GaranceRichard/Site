...
## 2026-03-26 - Les reruns Playwright locaux peuvent encore echouer en `spawn EPERM` sous Windows
- Contexte: pendant l'investigation d'un echec CI E2E, un rerun local cible via `npm run test:e2e -- --grep ...` a casse avant l'execution des tests.
- Symptomes: `Error: spawn EPERM` au demarrage de Playwright, sans resultat de test exploitable.
- Cause racine: contention ou blocage d'environnement Windows sur le spawn Node/Playwright, distinct de la panne fonctionnelle observee en CI Linux.
- Decision durable: sur ce repo, un `spawn EPERM` local Playwright sous Windows doit etre traite comme un blocage d'environnement recurrent; ne pas le confondre avec le bug fonctionnel cible et conserver une voie d'analyse par lecture de code, CI ou rerun dans un environnement qui demarre.
- Verification demandee: si un rerun local Playwright echoue en `spawn EPERM`, le statut doit distinguer explicitement blocage d'environnement et etat du bug fonctionnel; la source de verite reste le rerun du check E2E reel dans un environnement capable de lancer Playwright.

## 2026-03-26 - Le formatage Black doit etre revalide explicitement apres refactor de fichiers Python longs
- Contexte: un refactor backend valide fonctionnellement et au gate local a tout de meme casse en CI sur `python -m black --check --diff --verbose .`.
- Symptomes: CI rouge uniquement sur `backend/contact/text_exchange.py`, avec un diff Black Linux alors que les autres checks etaient verts.
- Cause racine: verification locale orientee tests/gate sans rerun explicite du check Black sur le fichier Python fortement remanie; les ecarts de wrapping restent faciles a manquer sur des fichiers longs.
- Decision durable: apres toute modification substantielle d'un fichier Python backend, rejouer explicitement `backend/.venv/Scripts/python.exe -m black --check --diff .` ou formatter le fichier cible avec Black avant commit; ne pas supposer que le gate de tests couvre le formatage.
- Verification demandee: si un fichier Python backend a ete touche, le statut ne peut pas etre considere vert tant que le check Black du repo n'a pas ete rerun et passe dans la session courante.
