...
export default function ContentExchangeManager({
  ...
  useEffect(() => {
    ...
    async function loadTemplate() {
      ...
      if (!token) {
        setStatus("error");
        setMessage("Connexion requise pour acceder au chargeur / extracteur.");
        onRequestLogin();
        return;
      }
  ...
  async function downloadFile(kind: "template" | "export") {
    if (!apiBase) {
      setStatus("error");
      setMessage("Configuration manquante : NEXT_PUBLIC_API_BASE_URL.");
      return;
    ...
    try {
      ...
      const filename = match?.[1] ?? `backoffice-exchange-${kind}.toml`;
      const blob = await response.blob();
      downloadBlob(blob, filename);
      setStatus("success");
      setMessage(
        kind === "template" ? "Canevas telecharge." : "Extraction telechargee.",
      );
    } catch (error) {
      setStatus("error");
      setMessage(error instanceof Error ? error.message : "Erreur inattendue.");
  ...
  async function onFilePicked(file: File) {
    ...
    try {
      ...
      setStatus("success");
      setMessage(
        `Import termine: ${(data as ImportResponse).references_count} reference(s) rechargee(s).`,
      );
    } catch (error) {
      setStatus("error");
      setMessage(error instanceof Error ? error.message : "Erreur inattendue.");
    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
      <section className="rounded-2xl border border-neutral-200 bg-white p-5 dark:border-neutral-800 dark:bg-neutral-900">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-sm text-neutral-600 dark:text-neutral-300">
              Le fichier TOML contient tous les contenus editables du site et les references.
              Lors d&apos;un import, les references sont recreees avec une image factice.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            >
              {isDownloadingTemplate ? "Chargement..." : "Telecharger le canevas"}
            </button>
            <button
              type="button"
            ...
            >
              {isDownloadingExport ? "Extraction..." : "Telecharger l'extraction"}
            </button>
          </div>
        </div>
        ...
        <div className="mt-5 rounded-2xl border border-dashed border-neutral-300 p-4 dark:border-neutral-700">
          ...
          <p className="mt-2 text-sm text-neutral-600 dark:text-neutral-300">
            Le fichier est controle avant application. Si une section est invalide, aucun
            changement n&apos;est enregistre.
          </p>
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <span className="self-center text-xs text-neutral-500">
              Formats acceptes: `.toml` ou `.txt` UTF-8
            </span>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-neutral-200 bg-white p-5 dark:border-neutral-800 dark:bg-neutral-900">
        <div>
          ...
          <p className="mt-2 text-sm text-neutral-600 dark:text-neutral-300">
            Utilisez ce format comme base de preparation. L&apos;ordre des blocs de references
            definit l&apos;ordre d&apos;affichage final.
          </p>
        </div>
